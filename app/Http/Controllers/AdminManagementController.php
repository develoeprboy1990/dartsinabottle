...
public function orderDetail($id,$type=null){
    ...
    $billing = SubscriptionBilling::where('subscription_id',$order_detail->id)->first();


    if($type == "shipped"){
        //Shipped 
        return view('user.admin.order-detail-shipped',['order_detail'=>$order_detail,'order_product'=>$order_product,'order_notes'=>$order_notes,'product_to_customers'=>$product_to_customers,'billing'=>$billing]);

    }elseif($type == "cancelled"){
        //Cancelled
        return view('user.admin.order-detail-cancelled',['order_detail'=>$order_detail,'order_product'=>$order_product,'order_notes'=>$order_notes,'product_to_customers'=>$product_to_customers,'billing'=>$billing]);

    }
    else{
        //Pending Shipment
        return view('user.admin.order-detail',['order_detail'=>$order_detail,'order_product'=>$order_product,'order_notes'=>$order_notes,'products'=>$products,'product_to_customers'=>$product_to_customers,'billing'=>$billing]);
    } 
